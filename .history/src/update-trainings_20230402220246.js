...
const updateEsicTrainingWithDigiformaData = async (item) => {
  ...
  data['prix'] = `À partir de ${prixFormatte.toLocaleString("en-US").replace(/,/g, ' ')} €`; //`À partir de ${prixFormatte} €`;
      ...
      if(digiformaItemData) {
        ...
        if(digiformaItemData.costs  && digiformaItemData.costs.length) {
            ...
            if (cost !== 0 ) {
              ...
              data['prix'] = `À partir de ${prixFormatte.toLocaleString("en-US").replace(/,/g, ' ')} €`; 
            }
