...
const updateEsicTrainingWithDigiformaData = async (item) => {
  const data = {};
  data['delai'] = DELAI_ACCES("2 Semaines");
      ...
      if(digiformaItemData) {
        ...
        if(digiformaItemData.accessDelay) {
          const {accessDelay, accessDelayUnit} = digiformaItemData;
            `${accessDelay}${(accessDelayUnit === 'WEEKS' || accessDelayUnit === null) ? ' Semaines': ''} ${accessDelayUnit === 'HOURS' ? ' Heures': ''} ${accessDelayUnit === 'MONTHS' ? ' Mois': ''}`.trim();
          data['delai'] =
        }
