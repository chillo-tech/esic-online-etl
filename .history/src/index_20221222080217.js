...
const save =  async (program, index) => {
  try {
    const response = await fetch(`${API_URL}/formations`, {
      method: 'post',
      body: JSON.stringify({...mapped, status: "published"}),
      headers: HEADERS
    });
    const {status} = await response;
    console.log(index, mapped.digiforma_id, mapped.libelle, status)
  } catch (ex) {
    console.error({ex});
    process.exit(1);
  }
    ...
    paginatedProgramsToSave.forEach((group, index) => {
        ...
        setTimeout(function () {
          group.forEach((item, groupIndex) => {
            const mapped = mapObject(program, PROGRAM_KEY_MAP);
            save(mapped, `${index}-${groupIndex}`)
          })
        }, interval * index);
    })
  } catch (ex) {
