...
const updateEsicTrainingWithDigiformaData = async (item) => {
  ...
  data['prix'] = numberWithCommas(prixFormatte); //`À partir de ${prixFormatte} €`;
    ...
    if (digiformaItemResponse.data.data) {
      ...
      if(digiformaItemData) {
        ...
        if(digiformaItemData.costs  && digiformaItemData.costs.length) {
            console.log({costs: digiformaItemData.costs});
            const firstCost = digiformaItemData.costs[0];
            let cost = firstCost.cost;
            if (cost !== 0 ) {
              cost = `${cost}`;
              const prixFormatte = cost.substring(0, cost.indexOf(',') > -1 ? cost.indexOf(','): cost.length).substring(0, cost.indexOf('€') > -1 ? cost.indexOf('€'): cost.length);
              data['prix'] = numberWithCommas(prixFormatte); 
            }
...
*/
    sortedEsicFormations.forEach((training)=> {
