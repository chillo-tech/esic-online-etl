    ...
    if (digiformaItemResponse.data.data) {
      ...
      if(digiformaItemData) {
        ...
        if(digiformaItemData.mentoring) {
          data['formateurs'] =  digiformaItemData.mentoring;
        }

        if(digiformaItemData.accessDelay) {
          const {accessDelay, accessDelayUnit} = digiformaItemData;
          data['delai'] =  `${accessDelay}${(accessDelayUnit === 'WEEKS' || accessDelayUnit === null) && ' Semaines'} ${accessDelayUnit === 'HOURS' && ' Heures'} ${accessDelayUnit === 'MONTHS' && ' Mois'}`;
        }
...
(async () => {
  try {
    ...
    const esicResponse = await axios.get(`${PROD_API_URL}/formations?fields=id,libelle,digiforma_id&limit=1000`, { headers: HEADERS });
